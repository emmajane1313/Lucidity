...
const Mensajes: FunctionComponent<MensajesProps> = ({
  ...
  return (
    <div className="relative w-full h-full flex flex-row justify-between items-start p-3 bg-black gap-4 overflow-y-scroll rounded-sm">
      <div className="relative w-full h-full flex items-start justify-start flex-col gap-3 overflow-scroll">
        <div className="relative w-full text-sm md:text-2xl font-count text-white uppercase flex h-fit">
          {dict.Home.agent}
        </div>
                  ...
                  >
                    ...
                    valor?.usuario !== Usuario.NewFlujo ? (
                      <div className="relative w-fit flex h-fit flex-col gap-2 items-start justify-start">
                        <div
                          className={`relative w-fit p-2 rounded-md h-fit flex items-center justify-center whitespace-pre-line break-all`}
                      ...
                      <div className="relative w-fit h-fit max-w-full flex flex-wrap gap-3">
                        {valor?.flujos?.map((flujo, indice) => {
                          return (
                            ...
                            >
                              <div className="relative w-20 sm:w-40 h-20 sm:h-40 rounded-md bg-black border border-brillo rounded-md">
                                <Image
                      ...
                      <div className="relative w-fit h-fit max-w-full flex flex-col gap-3 pb-4">
                        ...
                        <div className="relative w-full h-fit flex items-start justify-start overflow-y-scroll bg-gris border border-ligero rounded-md p-2">
                          <div className="relative sm:w-[50vw] w-20 h-96 text-sm">
      ...
      <div className="relative w-fit h-full flex flex-col gap-2 items-start justify-start overflow-y-clip">
        {Array.from({ length: 2000 }).map((_, i) => {
          return (
            ...
            >
              <div className="relative w-1 sm:w-4 h-1 sm:h-4 flex"></div>
            </div>
          );
        })}
      </div>
      <div className="relative w-full h-full flex items-start justify-start flex-col gap-3 overflow-scroll">
        <div className="relative w-full text-sm md:text-2xl font-count text-white uppercase justify-end flex h-fit">
          {dict.Home.user}
          {user && `: ${user}`}
        </div>
              ...
              ?.map((valor, indice) => {
                return (
                  ...
                  >
                    <div className="relative w-fit flex h-fit flex-col gap-2 items-start justify-start">
                      <div
                        className={`relative w-fit p-2 rounded-md h-fit flex items-center justify-center  break-all`}
                      >
                        {valor?.contenido}
                      </div>
                      {valor?.action && (
